...
const AftPriChat = () => {
      ...
      const fetchMessages = async () => {
        try {
          const response = await fetch(
            `http://localhost:5000/api/apchat/${selectedSchool}`
          );
  ...
  const handleSendMessage = async () => {
    if (newMessage.trim()) {
      try {
        const response = await fetch(
          "http://localhost:5000/api/apchat/messages",
          {
            method: "POST",
    ...
    <div className="flex h-screen m-0 p-0">
      ...
      <div className="flex flex-col flex-grow h-full bg-[#FFF1DB] -mt-4">
        ...
        <div className="sticky bottom-0 bg-white border-t border-gray-300 px-4 py-3 flex items-center space-x-4 shadow-md -mb-4">
          ...
          <button
            className="px-4 py-2 bg-blue text-white rounded-full hover:bg-blue hover:scale-105 transform transition-transform duration-150 flex items-center focus:outline-none"
